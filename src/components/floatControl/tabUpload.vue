...
import { geometryFunction } from 'src/utils/openLayers'
...
export default defineComponent({
  ...
  setup() {
    ...
    const removeEvent = async (files) => {
      files.forEach(async (file) => {
        const layerIndex = unref(uploadList).findIndex((u) => u.file.__key === file.__key)
        if (layerIndex !== -1) {
          const layer = unref(map)
            .getLayers()
            .getArray()
            .find((l) => l.get('id-upload') === unref(uploadList)[layerIndex].id)
    ...
    const saveFile = async () => {
      const selectedFiles = unref(uploadList).filter((up) =>
        unref(checkboxList).includes(up.file.__key),
      )
    ...
    const detailFile = async (index) => {
      ...
      if (layer) {
        ...
        if (features.length > 1) _extent = layer.getSource().getExtent()
        else {
          features.forEach((f) => {
