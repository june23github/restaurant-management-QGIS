...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const continueLineMsg = computed(() => $t('Click to continue drawing the line'))
    const continuePolygonMsg = computed(() => $t('Click to continue drawing the polygon'))
    ...
    const clearControl = () => {
      ...
      if (unref(measureTooltipElement)) {
        unref(measureTooltipElement)?.parentNode?.removeChild?.(unref(measureTooltipElement))
      }
    }
    const addInteraction = (type) => {
      ...
      drawEnd.value = unref(draw).on('drawend', function (evt) {
        ...
        drawList.value[unref(drawList).length] = {
          ...
          icon: geometryType instanceof LineString ? 'straighten' : 'img:icons/area.png',
          time,
          id: time,
    ...
    const deleteDraw = (index = -1) => {
      if (index !== -1) {
        ...
        unref(source).clear()
        document.querySelectorAll('div.ol-tooltip-static').forEach((d, idex) => {
          if (idex > 0) d.remove()
        })
        drawList.value = []
      }
    ...
    const downloadDraw = async (index = -1) => {
      if (index !== -1) {
        ...
        if (feature) {
          ...
          downloadLink.href = 'data:text/json;charset=utf-8,' + encodeURIComponent(geoJsonData)
    ...
    const createHelpTooltip = () => {
      if (unref(helpTooltipElement)) {
        unref(helpTooltipElement)?.parentNode?.removeChild?.(unref(helpTooltipElement))
      }
