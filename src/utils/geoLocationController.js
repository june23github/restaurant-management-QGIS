...
class GeoLocationController {
  constructor(option = {}) {
    ...
    this.zoomToLocation = _debounce((coordinates) => {
      console.log('zoomToLocation', coordinates)
  ...
  updateGeolocation() {
    ...
    const coordinates = transform(currentLocation, 'EPSG:4326', this.map.getView().getProjection())
  ...
  getCurrentLocation() {
    ...
    console.log('accuracyFeature', accuracyFeature)

    const positionFeature = this.positionFeature
    this.view = this.map.getView()

    const currentLocation = [108.153325, 16.075325]
    const coordinates = transform(currentLocation, 'EPSG:4326', 'EPSG:3857')
    //const coordinates = geolocation.getPosition(); // uncomment this to get the real location
