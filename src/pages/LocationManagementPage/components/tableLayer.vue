    ...
    >
      ...
      <template v-slot:top>
        ...
        <q-btn class="bg-white text-primary" rounded icon="add" style="margin-left: 10px">
          <q-tooltip anchor="center right" self="center start">{{ $t('Add layer') }}</q-tooltip>
          <popupLayer
      ...
      <template v-slot:body="propsLayer">
        <q-tr :props="propsLayer">
          ...
          <q-td key="type" :props="propsLayer">
            <q-badge
              :class="`bg-white text-${propsLayer.row.type === LAYER_TYPE[0] ? 'green' : 'red'}`"
            >
              {{ propsLayer.row.type }}
            </q-badge>
          </q-td>
